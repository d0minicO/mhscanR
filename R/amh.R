...
class1 <- function(df_row){
  ...
  if(df_row$strand == 1){
    ...
    df_row$altMH_count <- (countPattern(df_row$MH,seqa)+countPattern(df_row$MH,seqb))-1 # minus one that is the original MH identified
    ...
    df_row$altMH_count <- (countPattern(reverseComplement(DNAString(df_row$MH)),seqa)+countPattern(reverseComplement(DNAString(df_row$MH)),seqb))-1 # minus one that is the original MH identified
    df_row$add_delSize <- nchar(as.character(paste0(seqa, seqb), collapse=""))
  }
  return(df_row)
}
...
class2 <- function(df_row){
  ...
  if(df_row$strand== 1){
    ...
    df_row$altMH_count <- countPattern(df_row$MH,seqa)-1 # minus one that is the original MH identified
    df_row$add_delSize <- nchar(as.character(paste0(seqa), collapse=""))
  }else if(df_row$strand == 0){
    seqa <- as.character(Biostrings::getSeq(genome, df_row$LD_chr, df_row$LD_start, df_row$LD_stop))
    df_row$altMH_count <- countPattern(reverseComplement(DNAString(df_row$MH)),seqa)-1 # minus one that is the original MH identified
    df_row$add_delSize <- nchar(as.character(paste0(seqa), collapse=""))
  }
  return(df_row)
}
...
class3 <- function(df_row){
  ...
  if(df_row$strand == 1){
    ...
    df_row$altMH_count <- countPattern(df_row$MH,seqa) # DO NOT minus one because we did not scan over the part of the deletion that contained the original MH because it was contained within the sgRNA
    df_row$add_delSize <- nchar(as.character(paste0(seqa), collapse=""))
  }else if(df_row$strand == 0){
    seqa <- as.character(Biostrings::getSeq(genome, df_row$LD_chr, df_row$sg_stop, df_row$LD_stop))
    df_row$altMH_count <- countPattern(reverseComplement(DNAString(df_row$MH)),seqa) # DO NOT minus one because we did not scan over the part of the deletion that contained the original MH because it was contained within the sgRNA
    df_row$add_delSize <- nchar(as.character(paste0(seqa), collapse=""))
  }
